...
2. 结果没说「监听已经连着」就用 Monitor 工具跑结果里的监听命令（`timeout_ms: 1800000`，description「Cowart 画布请求」）。监听是画布请求送到本会话的唯一通道，没有它请求只会在服务里排队（不会丢，接上后补发）。Monitor 一次最多跑 30 分钟：监听快到点会自己打一行「马上用同一条命令重新启动监听」再退出，Monitor 结束的通知来了也一样，都用同一条命令马上重开，别等用户提醒。监听自己退出并说「重新打开画布」时（会话断开、服务换了版本），再 render 一次拿新的监听命令重跑。
