...
2. 结果没说「监听已经连着」就用 **Bash 工具后台运行**（`run_in_background: true`，description「Cowart 画布请求」）结果里的监听命令（带 `--once`）。它一直等着、不耗 token，画布上真来了请求才退出；退出的完成通知会叫醒你（通知里只有输出文件路径，内容要 Read 它）。没有定时器，服务换版本重启它也自己接上，会话挂一晚上都不会被叫醒。这是画布请求送到本会话的唯一通道，没在跑时请求在服务里排队（不会丢）。**别用 Monitor 跑它**：Monitor 最多 30 分钟，到点就叫醒会话，空闲时也白耗 token。监听退出并说「重新打开画布」时（会话跟服务断开了），再 render 一次拿新的监听命令重跑。
3. 之后别再为别的画布操作重新 render：画布自己每 1.6 秒同步，插进去的东西几秒内就显示。只有用户明确要重开 / 刷新时才再调。

## 3. 画布请求：监听叫醒你之后

监听退出的完成通知来了，先 Read 通知里的输出文件，里面有一行「Cowart 画布请求 #N「标题」（页「X」）：摘要 → 马上用 AskUserQuestion 问用户一句（…）」。这是**后台通知，不是用户在对话里说的话**，所以：

1. **先问再动**：读完就用 AskUserQuestion 问一句，选项按那行给的（「执行 / 跳过」，或「执行（免费本地模型）/ 执行（云端，消耗团队额度）/ 跳过」）。除了读输出文件，问之前不调别的工具，也不先去读详情。
...
5. 一次一条。同时来了几条（一次唤醒会带出同一批到的），按编号逐条问、逐条做（AskUserQuestion 一次可以问几个问题）。
6. **处理完再后台跑一次监听命令**（命令不变）接下一条：回 done / failed / skipped 时，监听没在跑的话 reply 的结果会提醒、带着命令。中间到的请求在服务里排队，监听一接上就补发。
7. **用户说「看画布」**（或问画布上有没有要处理的）：监听没在跑时请求在服务里排队，画布顶部写着「请求会排队 · 到对话里说「看画布」」。这时调 `list_cowart_requests`：等着的请求每条一行、带好要问的选项（跟监听那行一样），照样先问再做；列出来就算送到了，之后的监听不会再为它们叫醒你。结果末尾说监听没在跑的，照着再后台启动它。
...
- **用户粘来的画布索引**：画布上右键「拷贝索引」拷的，形如 `Cowart 画布 页面「角色设定」 shape:kL9x 图片「主角正面.png」 C:\…\主角正面.png`（多选时一行一个）。它指的就是画布上那张卡片：要看图直接 Read 后面那个路径，要改它 / 挪它 / 删它就用这个 `shape:` id 调上面的工具（那一页不归你时，先让用户在这个会话里说「接管 <页名>」）。视频右键还有「截取当前帧」，截下的那一帧是张普通图片卡片，素材名形如 `<视频名> 1.9s.png`。
...
- 画布请求先问后做；撤销了就不做。
- 不用 Monitor 跑监听（最多 30 分钟，到点叫醒会话、白耗 token）；监听只用 Bash 后台跑。
