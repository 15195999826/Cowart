...
// over the canvas service's localhost API, plus a status overlay. The page belongs to the
// Claude session that opened it («我是谁»). Each page of the canvas is the responsibility of
// one session at a time: a request clicked here goes to the session responsible for the
// page it was clicked on, and the overlay offers to hand the page to this session. Saves
// send only what changed in this page (see «Delta saves»), so several pages of the same
// canvas can be edited at once. The features both hosts share (AI 视频, AI 图片, video
// playback) live in adapters/shared/web.
(() => {
  'use strict'
  ...
  function createOverlay() {
    ...
    function me() {
      return (state.role && state.role.myName) || 'Claude'
    }
    ...
    function pillState() {
      if (state.serviceOnline === false) {
        return state.restarting
          ...
          : ['offline', 'Cowart 服务未连接：回到 Claude Code 说「打开 Cowart 画布」']
      }
      if (state.serviceOnline === null) return ['', '正在连接 Cowart 本地服务…']
      if (state.outdated) return ['outdated', '画布服务已更新：点这里刷新页面']
      const name = me()
      if (state.session === 'ended') return ['offline', `${name} 的会话已结束：回到 Claude Code 重新打开画布才能接上`, name]
      if (state.session === 'waiting') return ['waiting', `${name} · 等 Claude 会话重新连上…`, name]
      const duty = state.role && state.role.myPage ? `负责「${state.role.myPage}」` : '没负责任何页'
      if (!state.agentOnline) return ['waiting', `${name} · ${duty} · Claude 未在监听 · 请求会排队`, name]
      return ['agent', `${name} · ${duty} · Claude 已连接`, name]
    }
    ...
    function statusLabel(request) {
      ...
      switch (request.status) {
        ...
        case 'cancelled':
          return '↩️ 已撤销'
        default:
          if (!request.delivered) return '📥 已排队，等 Claude 连上'
          return request.session === config.session ? '👉 请到 Claude 对话里点「执行」' : `👉 请到${sessionName(request.session)}的对话里点「执行」`
      }
    }
