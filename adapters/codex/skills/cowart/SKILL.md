...
`get_cowart_canvas_state` 默认返回精简摘要（页、图形、素材路径、标注绑定、负责关系）；确需原始记录才传 `includeSnapshot: true`。`get_cowart_selection` 是本会话面板的选择。插入工具会计算位置、复制本地素材和保存，原文件不动；整理页面见下文「整理页面」。用唯一产物名，不覆盖已有素材。用户粘来的 `Cowart 画布 页面「X」 shape:… 图片「名字」 <本地路径>`（画布右键「拷贝索引」）指的就是那张卡片：Read 那个路径看图，用这个 shape id 调工具改它、挪它或删它。视频右键的「截取当前帧」把那一帧作为普通图片卡片放在视频右边。
