...
const RESOURCE_META = { ui: { prefersBorder: false, csp: { connectDomains: [], resourceDomains: [], frameDomains: [] } }, 'openai/widgetDescription': 'Cowart native canvas shared with Claude Code', 'openai/widgetCSP': { connect_domains: [], resource_domains: [], frame_domains: [] } }
    ...
    try {
      ...
      switch (params.name) {
        ...
        case 'get_cowart_request': {
          ...
          return textResult(`Cowart 请求 #${request.id} [${request.status}]\n${request.text}\n结果放回 pageId=${request.pageId}。`, request)
        }
