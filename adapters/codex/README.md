...
还没写。这份说明是交接给接手的人（或 Codex 自己）的：现状、要做的事、两边的差别。动手前先读仓库根目录的 [FORK.md](../../FORK.md)。
...
- 下面这些目前只在 Claude Code 里有（用法见 [adapters/claude/README.md](../claude/README.md)「另外补了上游没有的」）：
  ...
  它们的页面脚本和请求拼装都已经在 `adapters/shared/` 里，本来就是给两边共用的。Codex 适配层要做的，是把这些接进 Codex。

## 要做的事（建议顺序）

1. **MCP 代理**：参考实现是 `adapters/claude/lib/adapter.mjs`。
   - **上游当子进程**：用 `adapters/shared/upstream.mjs` 的 `UpstreamCowart` 起上游，工具原样转发。
   - **追加页面专用工具**：带 `_meta.ui.visibility: ['app']`，模型看不到，只给画布页面调。
   ...
   - **追加 `insert_cowart_video`**：现在写在 Claude 适配层里；视频探测和摆放在 `shared/video.mjs`，可以把工具本身也挪进 shared，两边共用。
...
2. **插件入口**：把 `.mcp.json`（或 `.codex-plugin/plugin.json`）改指适配层的启动脚本。这会改动上游文件，属于补丁点，要登记到 FORK.md，并在 `check-contract` 里加对应标记。
3. **打包**：Codex 从插件缓存直接运行，不会执行 `npm install`（上游 README 明确这么要求）。
   - 适配层的依赖（`@modelcontextprotocol/sdk`、`fractional-indexing`、`puppeteer-core`）要打成自包含的 bundle 并提交，做法同上游的 `mcp/generated/`。
   - `puppeteer-core` 比较大：网页截图可以按需加载，加载不到就给出明确提示。
4. **测试**：仿 `adapters/claude/scripts/smoke-test.mjs` 写 Codex 版冒烟测试：
   - 起代理，列工具，确认页面专用工具被隐藏。
   - 读画布资源，确认共用脚本已注入。
...
- **发送**：两边页面的流程一样：先调 `prepare_cowart_generation_request` 拿到请求文本，再用 `window.cowartMcp.sendFollowUpMessage` 发出。
...
- **素材和视频**：
  - Claude 那边的页面素材由本地网页服务提供，视频可以分段加载。
...
- [FORK.md](../../FORK.md)：开发准则、已核实的接缝、补丁点清单、分支与同步流程。
- [adapters/claude/README.md](../claude/README.md)：每个功能的用法和行为，Codex 版照着对齐。
- `adapters/claude/lib/adapter.mjs`：代理的参考实现（工具转发、页面专用工具、资源注入、写入保护）。
