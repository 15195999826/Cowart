// ZCode side of the Cowart canvas: a thin MCP server per session, the ZCode sibling of
// adapters/claude/lib/bridge.mjs. The canvas itself runs in the machine-wide canvas service
// (adapters/service), which serves it on localhost for any browser, makes one session at a
...
// 2026-09-14); ZCode's limit is unmeasured, so the bridge stays within the same budget and
// everything else lives in the cowart skill (adapters/zcode/skills/cowart). The smoke test
// checks the length.
export const INSTRUCTIONS_LIMIT = 2048
export const INSTRUCTIONS = [
  'cowart 是 Cowart 画布（tldraw 无限画布：图片、视频、HTML 草稿、标注）在 ZCode 里的入口。全机只有一张画布，由画布服务保管在 ~/.cowart/canvas，所有会话、所有项目共用；画布分页（tldraw 的页，左上角切换），一页一块板。',
  ...
  '其它 AI 按钮（按标注修改 / 按标注生图 / AI HTML / AI Slides / 照网页做 HTML）会让监听退出并唤醒你，输出是「Cowart 画布请求 #N」。这是后台通知、不是用户的话：按输出行的指引先用 AskUserQuestion 问一句（执行 / 跳过），问之前不调别的工具；选了执行才 get_cowart_request 看详情、reply_cowart_request 回状态，处理完再后台跑一次监听命令接下一条。',
...
const OWN_TOOLS = [
  {
    ...
    description:
      'Open (or re-open) the Cowart canvas in this session. With page (a page name) or shownPage, this session becomes responsible for that page: 「打开 Cowart 画布 角色设定」 / 「接管 角色设定」 → page "角色设定" (created when missing); 「接管这页」 → shownPage: true. Without either, the canvas opens and no page changes hands. Returns a localhost URL for the user to open in a browser, the background listener command that delivers canvas requests to this session, and who is responsible for which page.',
    inputSchema: {
      type: 'object',
      properties: {
        page: { type: 'string', description: 'Page to take responsibility for, by name (e.g. 角色设定); created if missing. The canvas opens on it.' },
        shownPage: { type: 'boolean', description: 'Take responsibility for the page this session\'s canvas page currently shows (「接管这页」).' },
        sessionName: {
          type: 'string',
  ...
  },
  {
    ...
    description: 'Read a canvas request routed to this session (full prompt plus ZCode host notes). Read-only; confirm with the user before acting on it.',
...
// ZCode does not pass a session id to the MCP servers it spawns (checked 2026-09-14: no
// ZCODE_*/ZAI_* session variable), so one bridge process is one canvas session; tests and the
// installer set COWART_SESSION_ID. Future ZCode versions naming one can slot in here.
function sessionId(env = process.env) {
  const raw = env.COWART_SESSION_ID || env.ZCODE_SESSION_ID || env.ZAI_SESSION_ID || ''
...
function generationNote(request) {
  ...
  if (request.kind === 'web') {
    return '这是网页复刻请求：先 Read 请求里的整页截图（很长时分段看），有标注的话逐张 Read 标注的局部截图，再读渲染后的页面代码；要看真实的颜色、字体、间距，或按坐标找标注指的元素，可以用 Skill 工具加载 control-browser（browser-use）打开原网址对照，读计算后的样式、用 document.elementFromPoint 定位。做成单文件 HTML 后按请求调用 insert_cowart_html_draft。'
  }
...
function renderResult(opened, session) {
  ...
  const lines = [
    ...
    opened.listenerConnected
      ...
      : `2. 用 Bash 工具启动画布请求监听，必须后台运行（run_in_background: true），命令：\n   ${listenCommand}\n   它收到一条画布事件就退出并唤醒你（通知里带输出文件路径）：读输出，按那行的指引先用 AskUserQuestion 问用户（执行 / 跳过），处理完再这样启动一次接下一条。`,
    '画布里点 AI 按钮时就是这样被唤醒的：「Cowart 画布请求 #N」→ 先问用户，选了执行再 get_cowart_request 看详情照做。',
...
export async function startZCodeBridge() {
  const log = (message) => process.stderr.write(`[cowart-zcode] ${message}\n`)
  // ZCode spawns this entry only from its own MCP config (no entrypoint variable to gate on,
  // unlike Claude Code's CLAUDE_CODE_ENTRYPOINT); nothing to disable here.
  const server = new Server({ name: 'cowart', version: VERSION }, { capabilities: { tools: {} }, instructions: INSTRUCTIONS })

  const session = sessionId()
  const service = new CanvasServiceClient({
    host: HOST,
    session,
    cwd: process.cwd(),
